...
def transform(data:dict) -> dict:
    SITE_LOGGER.info("TRANSFORM PROCESSED DATA ---------------")

    processed_data = data['raw_data']

    SITE_LOGGER.info("Creating site name and serial columns")
    processed_data['site'] = site["name"]
    processed_data['buoy_id'] = site["serial"]

    SITE_LOGGER.info("Converting timestamp")
    processed_data = ProcessEagleIOData.convert_to_datetime(processed_data, old_col="ts", new_col="timestamp", drop_old=True)   

    SITE_LOGGER.info("Converting units")
    processed_data = ProcessEagleIOData.convert_units(processed_data, SITE_ECM_PARAMETERS)
    
    SITE_LOGGER.info("Correcting directional data to true north")
    processed_data = ProcessEagleIOData.correct_true_north(processed_data,
    ...
    processed_data = ProcessEagleIOData.interpolate_latlon(processed_data)

    SITE_LOGGER.info("Creating time unix column")
    processed_data = ProcessEagleIOData.process_time_unix_column(processed_data, method='create')
    
    SITE_LOGGER.info("Conforming columns to AusWaves format")
    processed_data = ProcessEagleIOData.conform_cols_to_auswaves(processed_data, strip=True)
    
    SITE_LOGGER.info("Creating missing AusWaves columns with NaN values")
    processed_data = ProcessEagleIOData.create_missing_columns(processed_data)
    
    SITE_LOGGER.info("Dropping unwanted columns")
    processed_data = ProcessEagleIOData.drop_unwanted_columns(processed_data)

    SITE_LOGGER.info("Filling missing values with -9999")
    processed_data = ProcessEagleIOData.fill_nan_9999(processed_data)
    
    SITE_LOGGER.info("Reordering columns to match AusWaves format")
    processed_data = ProcessEagleIOData.reorder_cols(processed_data)

    SITE_LOGGER.info("Conforming columns to AusWaves format for loading")
    processed_data = ProcessEagleIOData.conform_cols_to_auswaves(processed_data, strip=False)

    SITE_LOGGER.info("Splitting data into daily dataframes")
    processed_data_daily = ProcessEagleIOData.split_data_daily(processed_data)

    SITE_LOGGER.info("Conforming timestamp to AusWaves format")
    processed_data_daily = ProcessEagleIOData.conform_timestamp_format_auswaves(processed_data_daily)
...
def load(data_daily):
    ...
    data_daily = cwb_s3.generate_daily_csv_keys(data_daily, site, vargs.enable_region_folder_structure)
